...
from lenlab.controller.discovery import NoLaunchpad, discover, find_launchpad
...
def test_discover(lenlab):
    discover(
        lenlab, [PortInfo("ttySmock0", ti_vid, ti_pid), PortInfo("ttySmock1", ti_vid, ti_pid)]
    )
    assert len(lenlab.port_infos) == 2
    assert lenlab.port_infos[0].name == "ttySmock0"
...
class Spy(QSignalSpy):
    ...
    def is_single_message(self, __class):
        return isinstance(self.get_single_arg(), __class)


def test_no_launchpad(lenlab):
