...
static void volt_initPoints(struct Packet *const self)
{
    self->label = 'L';
    self->code = 'v';
    self->arg = 0;
...
void volt_startLogging(uint32_t interval)
{
    ...
    if (interval) { // start logging
        if (DL_Timer_isRunning(VOLT_TIMER_INST)) {
            DL_Timer_stopCounter(VOLT_TIMER_INST);
        }

        volt_initPoints(&self->points[0].packet);
        volt_initPoints(&self->points[1].packet);
        ...
        terminal_transmitPacket(&volt.points.packet);
...
{
    uint16_t point[2] = (uint16_t *) volt.points[volt.ping_pong].payload[volt.point_index];
    ...
    if (other->done) {
        ...
        if (volt.point_index >= LENGTH(volt.points.payload)) {
            DL_Timer_stopCounter(VOLT_TIMER_INST);
        }
    }
}
